...
function filterPrivateData(employeeRecords) {
  const nonPrivateRecords = employeeRecords.map((employeeRecords) => ({name: employeeRecords.name, occupation: employeeRecords.occupation, email: employeeRecords.email,}));
